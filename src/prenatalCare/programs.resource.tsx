...
export const useAttentions = (patientUuid: string) => {
  ...
  const { data, error, isValidating } = useSWR< EncounterResponse>(
  ...
  const { data: detailedEncounters, error: detailedError } = useSWRImmutable(
    encounterUuids.length > 0
      ? encounterUuids.map((uuid) => `${restBaseUrl}/encounter/${uuid}?v=custom:(encounterDatetime,form:(uuid,display),obs:(uuid,display))`)
      : null,
    async (urls) => {
